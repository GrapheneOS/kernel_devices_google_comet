...
if [ -n "${KLEAF}" ]; then
  exec tools/bazel run --config=comet --config=fast //private/devices/google/comet:zuma_comet_dist
fi
...
export LTO=${LTO:-thin}
export BUILD_CONFIG=private/devices/google/comet/build.config.comet
...
export CORE_GKI_FRAGMENT=private/devices/google/comet/build.config.comet.gki.fragment
